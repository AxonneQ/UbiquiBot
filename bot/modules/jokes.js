const apisrc = [
    { name: 'chuck', protocol: 'http', address: 'http://api.icndb.com/jokes/random', extract: ['joke'] },
    {
        name: 'misc',
        protocol: 'https',
        address: 'https://jokes.guyliangilsing.me/retrieveJokes.php?type=dadjoke',
        extract: ['joke'],
    },
    { name: 'mom', protocol: 'https', address: 'https://api.yomomma.info/', extract: ['joke'] },
    {
        name: 'prog',
        protocol: 'https',
        address: 'https://sv443.net/jokeapi/v2/joke/Programming',
        extract: ['joke', 'setup', 'delivery'],
    },
    ...
    //Dark jokes, (need to implement toggle per server basis, turn off by default)
    {
        name: 'dark',
        protocol: 'https',
        address: 'https://sv443.net/jokeapi/v2/joke/Dark',
        extract: ['joke', 'setup', 'delivery'],
    },
];
...
function randomizeJoke(channel, args) {
    ...
    if (jokeAPI === undefined) {
        channel.send(`No category named '${args[0]}'`);
        return;
    }

    var http_s = require(jokeAPI.protocol);

    http_s.get(jokeAPI.address, (res) => {
...
function searchKey(object, value) {
    ...
    for (var key in object) {
        ...
        if (key === value) {
            // if the key is what we look for
            return object[value]; // return it's value
        } else if (typeof object[key] === 'object') {
            // else if the value of the key is an object
